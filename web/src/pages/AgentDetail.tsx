  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-white/30">{t.detail.loading}</p>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center text-center px-6">
        <div>
          ...
          <p className="text-white/40 mb-6">{t.detail.notFound}</p>
          <Link to="/agents" className="text-primary hover:underline">&larr; {t.detail.backToMarket}</Link>
        </div>
      </div>
    )
  }
            ...
            <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-8">
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 bg-white/[0.04] border border-white/[0.07] rounded-full text-white/40 text-sm">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-5">
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-6 sticky top-24">
              <div className="text-center mb-6">
                ...
                <p className="text-white/35 text-sm">{t.detail.creditsPerCall}</p>
              </div>
              ...
              >
                {user ? t.detail.startChat : t.detail.loginToChat}
              </button>

              {!user && (
                <p className="mt-4 text-center text-white/25 text-xs">
                  <button onClick={() => setAuthModal('login')} className="text-primary hover:underline">{t.nav.login}</button>
                  {` ${t.detail.loginOrRegister} `}
                  <button onClick={() => setAuthModal('register')} className="text-primary hover:underline">{t.nav.register}</button>
                  {` ${t.detail.toStart}`}
                </p>
              )}
              ...
              <div className="mt-6 pt-5 border-t border-white/[0.06] space-y-2.5">
                {[
                  { label: t.detail.status, val: agent.status },
                  { label: t.detail.model, val: agent.model_name || '-' },
                  { label: t.detail.maxTokens, val: String(agent.max_tokens) },
                ].map((r) => (
            ...
            {related.length > 0 && (
              <div className="bg-white/[0.02] border border-white/[0.07] rounded-2xl p-5">
                <p className="text-white/30 text-xs font-mono mb-3 tracking-wider">// {t.detail.related}</p>
                  ...
                  {related.map((r) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-white/30 text-xs">{r.cost} {t.agentCard.credits}</p>
                      </div>
                    </Link>
                  ))}
                </div>
